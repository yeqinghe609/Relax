...
CKPT_ARGS=(
   --hf-checkpoint ${MODEL_DIR}/Qwen3.6-35B-A3B-fp8
   --ref-load ${MODEL_DIR}/Qwen3.6-35B-A3B
   --megatron-to-hf-mode bridge
   --warm-hf-checkpoint-page-cache

   #--load ${MODEL_DIR}/Qwen3.6-35B-A3B-fp8-save
   --save ${MODEL_DIR}/Qwen3.6-35B-A3B-fp8-save
   --save-interval 200
...
if [ ${MODE} = "async" ]; then
    ray job submit ${RAY_NO_WAIT:+--no-wait} --address="http://127.0.0.1:8265" \
       ...
       "${PERF_ARGS[@]}" \
       "${SGLANG_ARGS[@]}" \
       "${MISC_ARGS[@]}"  2>&1 | tee log/qwen36-35B-A3B-fp8-gpu8-async-${now}.log
else
    ray job submit ${RAY_NO_WAIT:+--no-wait} --address="http://127.0.0.1:8265" \
       ${WORKING_DIR:+--working-dir "${WORKING_DIR}"} \
       --runtime-env-json="${RUNTIME_ENV_JSON}" \
       ...
       "${PERF_ARGS[@]}" \
       "${SGLANG_ARGS[@]}" \
       "${MISC_ARGS[@]}"  2>&1 | tee log/qwen36-35B-A3B-fp8-gpu8-colocate-${now}.log
fi
