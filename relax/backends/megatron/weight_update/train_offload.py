...
# memory. Manual offload never touched these; TMS's whole-pool pause happened to hide
# them, which is why manual offload showed a larger residual on MoE models.
...
def torch_memory_saver_preloaded() -> bool:
    """Return True when the ``torch_memory_saver`` hook ``.so`` is LD_PRELOAD'ed.

    Only in that case may we touch ``torch_memory_saver`` — otherwise even reading
    ``memory_margin_bytes`` triggers ``_ensure_initialized()`` -> AssertionError
    ("invalid LD_PRELOAD"). When it is not preloaded we fall back to the manual
    selective offload, which needs no ``.so``.
    """
    return "torch_memory_saver" in os.environ.get("LD_PRELOAD", "")


class _OffloadStrategy(Protocol):
    """Structural interface implemented by the TMS and manual offload strategies.

    A ``Protocol`` (not an ABC) keeps this composition-over-inheritance: the two
    strategies don't subclass anything, they just satisfy this shape, and the
    facade holds one via delegation.
    """
...
class _ManualOffloadStrategy:
    """Selective, application-level CPU offload of the live Megatron train state."""
    ...
    def _iter_optimizers(self) -> Iterator:
        """Yield every Megatron DistributedOptimizer (unwrapping ChainedOptimizer)."""
    ...
    def _iter_ddp_buffers(self) -> Iterator:
        """Yield every DDP flat buffer (params + expert-parallel) across model chunks."""
    ...
    def _clear_moe_dispatcher_caches(self) -> None:
        """Drop MoE token-dispatcher tensor caches so the pinned forward graph frees.
...
class MegatronTrainStateOffloader:
    """Facade over the TMS and manual offload strategies for colocate sleep/wake.
    ...
    def _init_torch_memory_saver(args) -> bool:
        """Configure torch_memory_saver; return True iff it is usable for offload."""
        from torch_memory_saver import torch_memory_saver
    ...
    def offload(self) -> None:
        """Offload the train state off the device (sleep). No-op when disabled."""
        if self.enabled:
            self._strategy.offload()

    def reload(self) -> None:
        """Reload the train state back onto the device (wake_up). No-op when disabled."""
        if self.enabled:
            self._strategy.reload()

    def disable_during_update(self) -> ContextManager:
        """Context manager wrapping weight update so offload bookkeeping stays consistent."""
        if not self.enabled:
            return nullcontext()
